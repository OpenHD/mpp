...
typedef struct {
    // Either 1 (low latency hack) or n of decoder buffers
    int num_buffers;
} OHDRunOptions;
...
void print_ohd_options(OHDRunOptions * ohdRunOptions) {
    printf("OpenHD\n");
    printf("N buffers: %d\n", ohdRunOptions->num_buffers);
}
void parse_ohd_options(OHDRunOptions * ohdRunOptions, int argc, char * argv[]) {

    static const char optstr[] = "b:";
    int c;
    while ((c = getopt_long(argc, argv, optstr, NULL, NULL)) != -1) {
        const char *tmp_optarg = optarg;
        switch (c) {
        case 'b':
            ohdRunOptions->num_buffers = atoi(tmp_optarg);
            break;
        }
    }
...
int main(int argc, char * argv[]) {
    ...
    OHDRunOptions ohdRunOptions;
    ohdRunOptions.num_buffers = 1;
    parse_ohd_options(&ohdRunOptions, argc, argv);
    print_ohd_options(&ohdRunOptions);
